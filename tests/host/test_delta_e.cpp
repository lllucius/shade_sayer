...
int main()
{
    static const DeltaECase kCases[] = {
    ...
    {
        const float got = color_math_delta_e_ciede2000(&kCases[i].l1, &kCases[i].l2);
        const float err = std::fabs(got - kCases[i].expected);
        if (err > kTolerance)
        {
            std::printf("case %zu failed: got=%.4f expected=%.4f err=%.4f\n", i + 1, got, kCases[i].expected, err);
        }
        assert(err <= kTolerance);
    }

    std::printf("delta_e regression passed (%zu cases)\n", sizeof(kCases) / sizeof(kCases[0]));
    return 0;
}
