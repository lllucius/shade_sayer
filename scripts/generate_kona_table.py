...
def parse_json_captures(path: pathlib.Path) -> List[KonaEntry]:
    ...
    for swatch in data.get("swatches", []):
        ...
        entries[kona_id] = KonaEntry(
            ...
            description=str(swatch.get("description", "")).strip(),
        )
...
def parse_json_synthetic(path: pathlib.Path) -> List[KonaEntry]:
    ...
    for swatch in data.get("swatches", []):
        ...
        entries[swatch_id] = KonaEntry(
            ...
            b=float(b),
            name=str(swatch.get("name", "")).strip(),
            nearest_name=str(swatch.get("nearest_name", "")).strip(),
            description=str(swatch.get("description", "")).strip(),
        )
...
def render_cpp(entries: List[KonaEntry], source_path: pathlib.Path, source_script: str = "generate_kona_table.py") -> str:
    ...
    # Build synthetic nearest-name table — only for synthetic entries with a nearest_name
    syn_name_rows = []
    for e in sorted_entries:
        if e.nearest_name:
    ...
    # Synthetic name table section
    if syn_name_rows:
        syn_table = f'''
const uint16_t kona_synthetic_name_count = {len(syn_name_rows)};

const kona_synthetic_name_t kona_synthetic_names[{len(syn_name_rows)}] = {{
{chr(10).join(syn_name_rows)}
}};
'''
    else:
        syn_table = '''
...
'''
    ...
    # Build description table — all entries with a non-empty description
    desc_rows = []
    for e in sorted_entries:
        if e.description:
            sanitized = e.description
            sanitized = sanitized.replace('\\', '\\\\')
            sanitized = sanitized.replace('"', '\\"')
            sanitized = sanitized.replace('\n', ' ')
            sanitized = sanitized.replace('\r', '')
    ...
    if desc_rows:
        desc_table = f'''
const uint16_t kona_description_count = {len(desc_rows)};

const kona_description_t kona_descriptions[{len(desc_rows)}] = {{
{chr(10).join(desc_rows)}
}};
'''
    else:
        desc_table = '''
...
// Synthetic nearest names: {len(syn_name_rows)}
